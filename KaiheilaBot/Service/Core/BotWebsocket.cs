...
namespace KaiheilaBot.Core
{
    public class BotWebsocket : IBotWebSocket
    {
        ...
        /// 初始化 Bot Websocket.
        /// </summary>
        public BotWebsocket(ILogService logger, IBotRequest botRequest, Shared sharedGlobal)
        {
            this.botRequest = botRequest;
            messageHub = sharedGlobal.messageHub;
            Log = logger;
        ...
        private async Task<string> GetWebsocketUrl()
        {
            ...
            var prepareRequest = botRequest.SetUrl("gateway/index").SetMethod(Method.GET);
            try
            {
                botRequest.AddParameter<int>("compress", 0);
            }
            catch
            {
                //已经存在相关Parameter
